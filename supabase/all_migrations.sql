...
-- ============================================================================
-- Migration 18: 20250209000018_fix_pricing_logic.sql
-- ============================================================================
...
BEGIN
  ...
  -- Pricing logic: <=5h hourly, 5-24h = 1 day, >24h = fullDays*day + remainder
  IF v_duration_hours <= 5 THEN
  ...
  ELSE
    ...
    v_full_days := v_duration_hours / 24;
    ...
    IF v_remainder = 0 OR v_remainder > 5 THEN
      v_billable_days := v_full_days + (CASE WHEN v_remainder > 5 THEN 1 ELSE 0 END);
    ELSE
  ...
  BEGIN
    ...
    VALUES (v_uid, -v_credits, format('Booking %s: %s → %s', v_booking_id, v_start_utc, v_end_utc));

    INSERT INTO public.audit_log (actor_user_id, action, entity_type, entity_id, metadata)
    VALUES (v_uid, 'booking.created', 'booking', v_booking_id,
      jsonb_build_object('credits_charged', v_credits, 'pricing_mode', v_pricing_mode));
  EXCEPTION
    WHEN unique_violation THEN
      RAISE EXCEPTION 'This car unit is already booked during the requested time window';
  END;

  SELECT coalesce(sum(delta), 0) INTO v_new_balance FROM public.credit_ledger WHERE user_id = v_uid;

  RETURN jsonb_build_object(
    'booking_id', v_booking_id, 'credits_charged', v_credits, 'balance_after', v_new_balance,
    'pricingMode', v_pricing_mode, 'durationHours', v_duration_hours
  );
END;
$$;
